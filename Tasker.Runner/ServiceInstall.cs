namespace Tasker.Runner
{
    ...
    internal static class ServiceInstall
    {
        ...
        private static void ApplyParameter<T>(InstallContext context, string parameter, Func<string, T> parse,
            Action<T> apply)
        {
            if (context == null)
            {
        ...
        public static void ApplyParameters(this ServiceInstaller installer, InstallContext context, bool install)
        {
            ...
            if (install)
            {
                ...
                ApplyParameter(context, DelayedAutoStartParameter, Boolean.Parse,
                    value => installer.DelayedAutoStart = value);
                ApplyParameter(context, DependedOnParameter,
                    value =>
                        installer.ServicesDependedOn =
                            value.Split(DependedOnDelimeters, StringSplitOptions.RemoveEmptyEntries));
            } //if
        }
